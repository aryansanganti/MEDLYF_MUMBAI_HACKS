...
import { Sparkles, Truck, Calendar, ArrowRight, RefreshCw, Loader2 } from "lucide-react";
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-xl font-bold">AI Resource Optimization</h2>
                        {regenerating && (
                                ...
                                <div key={idx} className="p-3 rounded-lg border border-border bg-card/50">
                                    ...
                                    <div className="flex gap-2">
                                        {hospital.days.map((day, dIdx) => (
                                            <div
                                                key={dIdx}
                                                className={`flex-1 flex flex-col items-center justify-center py-2 rounded-md border transition-all hover:scale-105 cursor-default ${day.status === 'CRITICAL' ? 'bg-red-50 border-red-100 text-red-600 dark:bg-red-900/20 dark:border-red-900/30' :
                                                    day.status === 'WARNING' ? 'bg-orange-50 border-orange-100 text-orange-600 dark:bg-orange-900/20 dark:border-orange-900/30' :
                                                        'bg-green-50 border-green-100 text-green-600 dark:bg-green-900/20 dark:border-green-900/30'
                                                    }`}
                                                title={`${day.predictedPatients} patients expected`}
                                            >
                                                <span className="text-[10px] font-semibold uppercase opacity-70">{day.day}</span>
                                                <span className="text-sm font-bold">{day.predictedPatients}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
